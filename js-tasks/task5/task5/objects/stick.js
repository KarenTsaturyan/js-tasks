...
class Stick {
    ...
    update (delta) {
        ...
        this.rotation=Math.atan2(a, b);// From (x1,y1) to (x2,y2) the direction is atan2(y2−y1,x2−x1) (-radians-)::pointing in the direction of the mouse. 
    }   

    // draws sprite every frame
    draw () {
        Canvas.drawImage(sprites.stick, this.position, this.origin, this.rotation, this.power);
    }
    increasePower(){
        this.power+=100;
        this.origin.x+=5;//move the stick back.
        if(this.origin.x >= 1200){//Max distance between ball and stick(cue).
            this.origin.x-=5;
            this.power = MaxPower;
        }
    }
    ...
    reposition(position){//called in gameWorld.js
